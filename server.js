...
app.post('/api/votes', async (req, res) => {
    ...
    const id = Math.random().toString(36).substring(2, 14); // Генерация id
...
app.post('/api/votes/:voteId/nominations', async (req, res) => {
    ...
    await pool.query('INSERT INTO nominations (id, vote_id, name, created_at, users_table) VALUES ($1, $2, $3, $4, $5)', [id, voteId, name, createdAt, usersTable]);
...
app.post('/api/nominations/:nominationId/candidates', async (req, res) => {
    ...
    const candidateId = Math.random().toString(36).substring(2, 14); // Генерация id
    const createdAt = new Date();
...
app.post('/api/sessions', async (req, res) => {
    ...
    const id = Math.random().toString(36).substring(2, 8); // Генерация id
    const code = Math.random().toString(36).substring(2, 8); // Генерация code
    const createdAt = new Date();
    const devices = 0;

    await pool.query('INSERT INTO sessions (id, name, created_at, devices, type, code) VALUES ($1, $2, $3, $4, $5, $6)', [id, name, createdAt, devices, type, code]);
...
app.patch('/api/sessions/:id/code', async (req, res) => {
    const { id } = req.params;
    const newCode = Math.random().toString(36).substring(2, 8); // Новый код
